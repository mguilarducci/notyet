...
- **`src/notyet/task/record.gleam`** — `TaskRecord` (the row to persist: `id`, `idempotency_key`, `wait_for`, `destination`, `visible_at`, `wait_until`, `created_at`). `visible_at` is the internal scheduler column (equals `wait_until` at creation). Dependency-free, breaking the `task`↔`batch` import cycle.
