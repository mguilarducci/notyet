...
- **`src/notyet/task/record.gleam`** — `TaskRecord` (the row to persist: `id`, `idempotency_key`, `wait_for`, `destination`, `wait_until`, `created_at`). Dependency-free, breaking the `task`↔`batch` import cycle.
