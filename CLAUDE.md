...
mist (HTTP) → wisp_mist adapter → router.handle_request(req, ctx)
            ...
            → web.middleware → wait.read  → sql.get_wait_by_idempotency_key(ctx.db)
...
- **`src/notyet/router.gleam`** — single dispatch point. Applies `web.middleware`, then matches `wisp.path_segments(req)` + `req.method`. Pattern: `["wait"], Post -> create`, `["wait"], _ -> 405`, `["wait", key], Get -> read`, `["wait", key], _ -> 405`, `_ -> 404`.
...
- **`src/notyet/wait.gleam`** — the feature module: request type (`WaitRequest`), decoders (`wait_decoder` + private `for_decoder`/`activity_decoder`), the `create` handler (returns `202` on enqueue, `429 + Retry-After` on shed), and the `read` handler (`200` with the resource, `404` on miss, `500` on query error) plus its private `row_to_wait`/`decode_data` row→read-model mapping.
- **`src/notyet/wait/view.gleam`** — the read-model: the `Wait` type and `encode` (the `GET /wait/{key}` response body). Pure — no `sql`/`pog` import — so `encode` is unit-testable without a DB; the handler is the only persistence seam.
...
`POST /wait` contract: requires an **`Idempotency-Key` header** that is a **UUID v4** (missing/empty/non-v4 → `422`, checked before the body; stored in canonical lowercase form so a repeat with different casing still deduplicates) and a JSON body `{"for": <duration string>, "activity": <uuid v4>, "data"?: <json object>}` → **`202 {"status": "accepted"}`** (ack-on-enqueue; no row echoed — a 202 confirms enqueue, not persistence, which happens asynchronously) when accepted into the write buffer, or **`429` + `Retry-After`** when the buffer is full and all in-flight insert slots are busy (load-shed). The wait is persisted **asynchronously** via the batched write-behind path (`status = 'accepted'`); a failed insert is logged and dropped (best-effort, no dead-letter). **Idempotency:** writes are deduplicated by `Idempotency-Key` (`UNIQUE` column + `ON CONFLICT DO NOTHING`); a repeat with the same key never creates a second row. A retry after a `429` is safe. `activity` is **required** and validated as a strict **UUID v4**; missing/non-string/non-UUID/non-v4 → `422`. Invalid/missing/non-duration `for` → `422`. Wrong method on `/wait` → `405`. `data` is optional; when present it must be a JSON object, stored as JSONB (absent → SQL `NULL`); a non-object `data` → `422`. `status` is a state machine seeded at `accepted` (the `waits.status` column defaults to `'accepted'` with `CHECK (status IN ('accepted','waiting'))`).

`GET /wait/{key}` contract: `{key}` is the `Idempotency-Key`, a **UUID v4** (validated and matched in canonical form; a non-v4 segment → `404` without a DB query). Returns **`200`** with the complete resource `{"id", "activity", "idempotency_key", "status", "for", "wait_until", "created_at", "data"?}` (timestamps RFC3339 UTC; `for` is the raw duration string; `data` is the stored JSON object, **omitted** when the wait had none) when the row exists, or **`404`** when it does not. Because the write path is async, a `GET` issued right after a `202` may `404` until the row is flushed — the read path queries Postgres only (`sql.get_wait_by_idempotency_key`) and never the coordinator buffer, so a miss is indistinguishable from "never existed". A query/connection error → **`500`**; wrong method on `["wait", key]` → **`405`**. Lookup is by the `UNIQUE idempotency_key` column (0-or-1 row). The read query casts UUIDs/`data` to `text` and formats timestamps as RFC3339 via `to_char` (squirrel cannot decode `timestamptz`); SQL `NULL` `data` is `COALESCE`d to `""` (the same empty-string sentinel the insert path uses via `NULLIF`), which `wait.read` maps back to an omitted field. `row_to_wait` re-validates the stored row (UUIDs, status, RFC3339 timestamps) with `let assert` — total against rows this service wrote through the constrained schema.
...
- Test layout mirrors `src/`. Focused unit tests per concern (`duration_unit_test`, `decoder_unit_test`, `json_value_unit_test`, `status_unit_test`, `view_unit_test`), DB integration tests (`sql_integration_test`, `batch_test`, `handler_test`), and a router-level integration test (`router_integration_test`).
